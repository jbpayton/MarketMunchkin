...
def daemon(once: bool = typer.Option(False, help="one loop iteration and exit")) -> None:
    ...
    def _rotation_task() -> str:
        ...
        board = ("DUTY: OPPORTUNITY BOARD. Cash is a position and deploying is not the goal. Rank the best candidates for the next 1-3 Runs "
                 ...
                 + ("Under Aggressive the board must include bullish AND bearish candidates expressed as bought calls and puts on underlyings whose one-contract premium fits $60-250 (price roughly $15-120, liquid chain); stock is the fallback for slow theses only." if get_style(j) == "aggressive" else ""))
        try:
            from . import screener as scr
...
@app.command()
def reevaluate(reason: str = typer.Option("operator request", help="why the arms and positions should be re-judged now")) -> None:
    """Queue a top-priority run that re-judges every armed entry and position against the rules in force (also fires on a style switch)."""
    from .journal import Journal
    from .styles import queue_reevaluation
    tid = queue_reevaluation(Journal(), reason)
    console.print(f"queued reevaluation as task #{tid}; the daemon runs it within a minute during market hours")
