...
<div class="card"><h2>Model</h2><div class="dim" style="font-size:13px;margin-bottom:8px">Any OpenAI-compatible chat server with tool calling. Read per run, so changes apply to the next run without a restart. Key goes to <code>.env</code> as LLM_API_KEY.</div><div id="llmbox">loading…</div></div>
...
<div class="card"><h2>Recent activity <span class="dim">last run ${esc((o.last_session_end||'').slice(11,16))} · every ${o.cadence.interval_min}m + events</span></h2>${o.events.map(e=>`<div class="ev"><span class="ts">${esc(e.ts.slice(5,16).replace('T',' '))}</span><span class="pill ${e.kind==='event'?'acc':e.kind==='error'?'bad':''}">${esc(e.kind)}</span> ${esc(e.text)}</div>`).join('')||'<div class="dim">nothing yet</div>'}</div>`;}
...
${d.decisions.length?`<div class="card"><h2>Decisions this run</h2>${d.decisions.map(x=>`<div class="row"><div><b>${esc(x.kind)}</b> ${esc(x.symbol)} ${esc(x.side||'')} ${x.qty??''} @ ${x.price??''} <span class="pill ${x.status==='filled'?'ok':x.status==='blocked'?'bad':''}">${esc(x.status)}</span><div class="dim" style="font-size:12px">${esc(x.thesis||'')}</div>${x.stop?`<div class="dim" style="font-size:12px">stop ${esc(x.stop)} · target ${esc(x.target||'')}</div>`:''}</div></div>`).join('')}</div>`:''}
